...
Για τις ανάγκες αυτού του ερωτήματος υλοποιήθηκε δικό μας πρόγραμμα `oddnums.c`, που υπολογίζει και τυπώνει τους περιττούς αριθμούς από το 0 εως το 1000.
...
Αντίθετα το **TimingSimpleCPU** εκτελεί πιο λεπτομερώς το εκάστοτε πρόγραμμα, ενώ παράλληλα εμπεριέχει όλες τις χρονικές καθυστερήσεις που θα συνέβαιναν σε ένα ρεαλιστικό σενάριο. Αυτή η αναμονή για την απόκριση του συστήματος έως ότου να εκτελέσει το επομενο *instruction* καθυστερεί την συνολική διαδικασία.
...
Όπως βλέπουμε η επιτάχυνση της **TimingSimpleCPU** από το 1GHz στα 5GHz είναι ελάχιστη και συγκεκριμένα ***0.62%***, καθώς, όπως προαναφέραμε, το μοντέλο περιορίζεται από τις αργές προσβάσεις στην μνήμη. Στην περίπτωση τώρα του μοντέλου **MinorCPU**, η επιτάχυνση είναι πολύ μεγαλύτερη ***6%***, επειδή με την αύξηση του ρολογιού επιταχύνονται τα στάδια του [**pipeline**](#minorcpu-pipeline) που δεν αφορούν προσβάσεις σε μνήμη.
...
Το σημαντικότερο κομμάτι της εργασίας ήταν σίγουρα η εξοικείωση με τον εξομοιωτή **gem5** και σε δεύτερο χρόνο με το **git** _control system_ και την online πλατφόρμα του **Github**, καθώς και με την γλώσσα **Markdown**.  
Όσον αφορά στον **_gem5_**, πρόκληση αποτέλεσε το "κακογραμμένο" **documentation** του, το οποίο κληθήκαμε να προσεγγίσουμε και να αναζητήσουμε μέσα σε αυτό πληροφορίες για πολύ εξειδικευμένα προγράμματα και τεχνολογίες.  
Επίσης, "εντρυφήσαμε" στην λογική να μην ακολουθούμε "τυφλά" οδηγίες, το οποίο αν και λίγο "αποθαρρυντικό" στην αρχή, εν τέλει, αποτέλεσε πολύ θετικό γεγονός σαν συνολική εμπερία. Για παράδειγμα, με την προτεινόμενη μέθοδο εγκατάστασης του _gem5_ προκύπτανε πολλαπλά προβλήματα, τα οποία, ευτυχώς, ξεπεράστηκαν με την κατάλληλη αναζήτηση άρθρων αλλά και _tutorials_. Αξίζει να σημειωθεί ότι δεν υπάρχει πληθώρα πληροφοριών σχετικά με τον _gem5_ στο διαδίκτυο.  
Στη συνέχεια, αναφορικά με το **_Github_** και την γλώσσα **_Markdown_**, η γνωριμία με αυτά τα "εργαλεία" ήταν ευχάριστη και η ενασχόληση με αυτά ακόμα πιο γόνιμη. Επαληθεύτηκε η λειτουργία τους ως εργαλεία, καθώς προσέφεραν μια χρηστική βοήθεια στο "στήσιμο" του _repository_ χωρίς να κάνουν την διαδικασία χρονοβόρα ούτε περίπλοκη.  
Συνολικά, ήταν μια αρκετά ενδιαφέρουσα εμπειρία με τα θετικά και τις δυσκολίες της!
